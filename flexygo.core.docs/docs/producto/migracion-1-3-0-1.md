# Migración de producto a Flexygo 1.3.0.1

La versión **1.3.0.1** de Flexygo Core introduce cambios estructurales importantes en los proyectos derivados: nuevo sistema de assets estáticos vía NuGet, carpeta `conf/` de configuración, sistema `buildTransitive`, nuevo `nuget.config` a nivel de solución y eliminación de los `install.map.json`, entre otros.
...
[:fontawesome-solid-file-zipper: Descargar migration-script.zip](../files/script.zip){ .md-button .md-button--primary }
